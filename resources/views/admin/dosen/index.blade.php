...
@section('content')
    <h1 class="text-3xl font-semibold mb-6 dark:text-white">Daftar Dosen</h1>
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <form action="{{ route('sync.dosen') }}" method="POST">
                                    @csrf
                                    <label for="tahunAkademik" class="block text-sm font-medium text-gray-700">Tahun
                                        Akademik:</label>
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <button type="button" onclick="toggleModal()"
                                            class="ml-4 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-gray-700 bg-gray-200 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                                            Batal
                                        </button>
                                    </div>
                                </form>
            ...
            <tbody>
                @foreach ($dosen as $dsn)
                    <tr
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{ $dsn->nama }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $dsn->nip }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $dsn->jurusan }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $dsn->prodi }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $dsn->email }}</td>
                        <td class="px-6 py-4 text-center space-x-2 relative whitespace-nowrap">
                            <button type="button" onclick="toggleActionModal({{ $dsn->id }})"
                            ...
                            <div id="actionModal-{{ $dsn->id }}"
                                ...
                                <form action="{{ route('dosen.makeAdmin', $dsn->id) }}" method="POST" class="block">
                                    @csrf
                                    <button type="submit"
                                        class="w-full py-2 px-4 bg-green-600 text-white text-sm rounded hover:bg-green-700 transition duration-200 ease-in-out">
