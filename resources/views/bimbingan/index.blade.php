        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($bimbingans as $bimbingan)
                <a href="{{ route('bimbingans.show', ['bimbingan_id' => $bimbingan->id_bimbingan]) }}"
                    class="dark:bg-gray-800 bg-white dark:text-white p-6 rounded-lg shadow-lg transform transition duration-300 hover:scale-105 hover:shadow-xl block">
                    ...
                    <div class="text-sm text-gray-600 dark:text-white mt-2">
                        @role('dosen')
                            <p><span class="font-medium">Mahasiswa:</span> {{ $bimbingan->mahasiswaBimbingan->nama }}
                                ({{ $bimbingan->mahasiswaBimbingan->nim }})
                            </p>
                        @endrole
                    ...
                    <div class="mt-4">
                        <span
                            class="text-xs 
                    @if ($bimbingan->status_bimbingan === 'selesai') bg-green-500 text-white dark:bg-green-400 dark:text-white 
                    @else 
                        bg-blue-500 text-white dark:bg-blue-400 dark:text-white @endif
