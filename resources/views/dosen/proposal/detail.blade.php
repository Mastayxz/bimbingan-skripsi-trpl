        ...
        <div class="bg-white border border-gray-300 shadow-lg rounded-lg p-6">
            ...
            <p class="text-gray-700 mb-4">
                @if ($proposal->file_proposal)
                    <a href="{{ $proposal->file_proposal }}" target="_blank" class="text-blue-500 hover:underline">
                        Unduh Proposal
                    </a>
                @else
                ...
                @if (count($proposal->listKomentar) > 0)
                    <ul class="list-disc pl-5">
                        @foreach ($proposal->listKomentar as $komentar)
                            <li class="text-gray-700 mb-2">{{ $komentar }}</li>
                        @endforeach
                    </ul>
                @else
            ...
            @role('dosen')
                ...
                @elseif ($proposal->status === 'disetujui' || $proposal->status === 'revisi')
                    <button onclick="openRevisiModal({{ $proposal->id_proposal }})"
                        class="py-2 px-4 bg-yellow-500 text-white rounded hover:bg-green-600">
                        Revisi
                    </button>
                    {{-- <form action="{{ route('dosen.proposal.revisi', $proposal->id_proposal) }}" method="POST"
                        class="inline-block">
                        @csrf
                        @method('PUT')
                        <button type="submit" class="py-2 px-4 bg-yellow-500 text-white rounded hover:bg-yellow-600">
                            Revisi
                        </button>
                    </form> --}}
