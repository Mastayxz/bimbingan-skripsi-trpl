    ...
    <div class="container mx-auto mt-8 px-4">
        ...
        @if (Auth::user()->hasRole('dosen'))
            <a href="{{ route('tasks.create', $bimbingan->id_bimbingan) }}"
                class="block mb-6 py-2 px-4 text-white bg-blue-600 rounded-lg hover:bg-blue-700">
                Tambah Task
            </a>
        @endif
            ...
            @foreach ($tasks as $task)
                <div class="bg-white shadow-lg rounded-lg p-6 hover:shadow-xl transition duration-200">
                    ...
                    <div class="mb-4">
                        ...
                        <span
                            class="font-semibold {{ $task->status == 'belum dikerjakan' ? 'text-red-500' : ($task->status == 'sedang direvisi' ? 'text-yellow-500' : 'text-green-500') }}">
                            {{ ucfirst($task->status) }}
                        </span>
                    </div>
                    ...
                    @if (Auth::user()->hasRole('mahasiswa'))
                        @if ($task->status == 'belum dikerjakan')
                            <!-- Hanya tampilkan link upload untuk tugas yang belum dikerjakan -->
                            <a href="{{ route('tasks.unggah', $bimbingan->id_bimbingan) }}"
                    ...
                    @if (Auth::user()->hasRole('dosen'))
                        @if ($task->link_mahasiswa)
                            <a href="{{ Storage::url($task->link_mahasiswa) }}" target="_blank"
                                class="text-blue-600 hover:text-blue-800 mt-4 inline-block">
                        ...
                        <div class="mt-4">
                            ...
                            <a href="{{ Storage::url($task->link_feedback_dosen) }}" target="_blank"
